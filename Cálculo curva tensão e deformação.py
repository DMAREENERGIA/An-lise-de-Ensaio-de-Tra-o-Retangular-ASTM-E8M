...
def processar_ensaio(dados_curva, w0, t0, L0, wf, tf, Lf, E_gpa, offset_pct):
    ...
    # Reta de Offset: y = E * (eps_abs - offset_abs)
    y_reta_offset = E_mpa * ((arr_eps - offset_pct) / 100.0)
    ...
    # Reta da Região Elástica (Origem)
    x_lin_max = (y_lim_max / E_mpa) * 100.0
    x_lin = np.linspace(0, x_lin_max, 100)
    y_lin = E_mpa * (x_lin / 100.0)
    ax.plot(x_lin, y_lin, 'k--', alpha=0.5, linewidth=2.5, label=f'Módulo E = {E_gpa:.0f} GPa')

    # Reta de Offset Móvel
    x_off_max = offset_pct + (y_lim_max / E_mpa) * 100.0
    x_off = np.linspace(offset_pct, x_off_max, 100)
    y_off = E_mpa * ((x_off - offset_pct) / 100.0)
    ...
    if len(cruzamentos) > 0:
        ax.scatter(deformacao_escoamento, tensao_escoamento, color='orange', s=280, zorder=5, 
...
def gerar_relatorio_pdf(resultados, w0, t0, L0, wf, tf, Lf, fig, E_gpa, offset_pct):
    ...
    # Inserção do autor no PDF (sem acentos para evitar erros na biblioteca FPDF padrão)
...
st.markdown("**Autor do Aplicativo:** Derli da Rosa (Graduado em Automação)")
st.markdown("---")
...
E_gpa = st.sidebar.slider("Módulo de Elasticidade - E (GPa)", min_value=0.0, max_value=250.0, value=200.0, step=5.0)
...
offset_pct = st.sidebar.slider(
    ...
    value=0.2, 
    step=0.01,
    help="Define o deslocamento paralelo da reta. Para aços o padrão é 0,2%."
)
...
if len(dados_ensaio) > 0:
    resultados, fig = processar_ensaio(dados_ensaio, w0, t0, L0, wf, tf, Lf, E_gpa, offset_pct)

    # NOVO LAYOUT: O gráfico agora ocupa 100% da largura superior e os resultados ficam abaixo
